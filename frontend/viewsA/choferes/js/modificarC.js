...
async function datachofer(){
    let chofer = [];
    
    await fetch(api+`/choferes/idchofer`)
    .then(response => response.json())
    .then((data) => chofer = data[0])
        Cemail.value = chofer.email;
        Cnombre.value = chofer.nombre;
        apellido.value = chofer.apellido;
        direccion.value = chofer.direccion;
        cedula.value = chofer.identificacion;
...
async function guardar(){
    ...
    xhr.onreadystatechange = function() {
        if (this.readyState == 4 && this.status == 200) {
            ...
            swal("¡Modificado!","Conductor Modificado Correctamente","success")
    ...
    xhr.open("POST", api+"/choferes/modificarC");
